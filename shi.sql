...
--
-- Host: 127.0.0.1
-- Tempo de geração: 14-Fev-2026 às 16:34
...
-- Procedimentos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_birthdaysupdate_save` (IN `pid_birthday` INT, IN `pdate_birthday` DATE, IN `pname_birthday` VARCHAR(64), IN `pesp_birthday` VARCHAR(264), IN `pdepartment` VARCHAR(264), IN `pbirthday_active` TINYINT, IN `pemail_recipient` VARCHAR(255))   BEGIN
    UPDATE tb_birthdays
    SET
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_birthdays_save` (IN `pdate_birthday` VARCHAR(264), IN `pname_birthday` VARCHAR(264), IN `pesp_birthday` VARCHAR(264), IN `pdepartment` VARCHAR(264), IN `pbirthday_active` TINYINT(4), IN `pemail_recipient` VARCHAR(255))   BEGIN
    DECLARE vid_birthday INT;
    
    INSERT INTO tb_birthdays (date_birthday, name_birthday, esp_birthday, department, birthday_active, email_recipient)
    VALUES(pdate_birthday, pname_birthday, pesp_birthday, pdepartment, pbirthday_active, pemail_recipient);
    
    SET vid_birthday = LAST_INSERT_ID();
    
    SELECT * FROM tb_birthdays WHERE id_birthday = vid_birthday;
END$$
...
INSERT INTO `tb_birthdays` (`id_birthday`, `date_birthday`, `name_birthday`, `esp_birthday`, `birthday_active`, `id_user`, `department`, `email_recipient`, `dt_birthday`) VALUES
...
(100, '2026-02-14', 'Parabenizando ao aniversariante(s)! Grande abraço da Shineray do Brasil !', 'Parabenizando ao aniversariante(s)! Grande abraço da Shineray do Brasil !', 1, 0, 'Geral', 'clecioshineraybrasil@gmail.com', '2026-02-14 13:47:03'),
(101, '2026-02-14', 'Parabenizando ao aniversariante(s)! Grande abraço da Shineray do Brasil !', 'Parabenizando ao aniversariante(s)! Grande abraço da Shineray do Brasil !', 1, 0, 'Geral', 'clecioshineraybrasil@gmail.com', '2026-02-14 14:08:25');
...
(87, 'Tecnologia da informações', '', '2026-02-14 15:33:15'),
...
(12, '', 'Modelo BCI', '#183e7a', '', 'modelo-bci', 1, 17, '2026-02-08 12:26:17'),
(14, '', 'Modelo BCI teste', '#fff700', '', 'modelo-bci-teste', 1, 17, '2026-02-08 13:38:21');
...
--
ALTER TABLE `tb_birthdays`
  MODIFY `id_birthday` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=106;
...
--
ALTER TABLE `tb_departments`
  MODIFY `id_department` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=101;
...
--
ALTER TABLE `tb_subscriptions`
  MODIFY `id_subscription` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=16;
